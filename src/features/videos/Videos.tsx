...
const Videos = (): React.JSX.Element => {
    ...
    return (
        ...
        >
            <h2 id="videos-heading" className="text-[30px] mb-5 mt-[30px] desktop:mt-[30px] wide:mt-[30px]">
                Videos & Tutorials
            </h2>
            <div className="flex flex-wrap justify-center mt-[10px] w-full desktop:w-full desktop:flex-wrap" role="list" aria-label="Video tutorials">
                {videosList &&
                    list.map((video, key) =>
                        video.open ? (
                            <div key={key} role="listitem" className="flex-[100%] m-[10px] desktop:flex-[30%] wide:flex-[30%]">
                                <iframe
                                    src={`https://www.youtube.com/embed/${video.videoId}`}
                                    title={`${(videosList as any)[video.videoId]?.title || "Video tutorial"}`}
                                    frameBorder="0"
                                    allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share"
                                    allowFullScreen={true}
                                    className="w-full border-0 h-[320px] desktop:h-auto wide:h-auto"
                                    aria-label={`${(videosList as any)[video.videoId]?.title || "Video tutorial"}`}
                                />
                            </div>
                        ) : (
                            <Button
